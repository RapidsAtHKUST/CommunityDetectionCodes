...
/** Community
 ...
 contenuti vengono memorizzate in un set, perch� operazioni tipiche eseguite
 sulle community sono intersezioni, unioni e verifiche di appartenenza. I set
 forniscono quindi la flessibilit� necessaria.
...
struct Community {
    ...
    /**
     ...
     vettore passato. Attenzione, perch� il vertex selector creato abbia senso,
    ...
    // ritengo opportuno calcolarli una volta per tutte anzich� chiamare le funzioni getCommunitySize(),
...
double Communities::flakeODF(CID s) {
    ...
    BOOST_FOREACH(Node v, *a->set) {
                    ...
                    while (!IGRAPH_VIT_END(vit)) {
                        if (ownership((long int) IGRAPH_VIT_GET(vit),
                                      s)) //verifica se i nodi appartengono alla community s
                            i_degree += 1.0;
                        IGRAPH_VIT_NEXT(vit);
                    }
...
double Communities::similarity(CID ia, CID ib) {
    ...
    double adjs = 0;
    BOOST_FOREACH(unsigned int v, *a->set)
                    BOOST_FOREACH(unsigned int w, *b->set)if (v != w && areConnected(v, w)) adjs++;
