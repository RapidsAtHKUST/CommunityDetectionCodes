...
bool Parameters::arrange() {
    ...
    if (num_nodes <= 0 || average_k <= 0 || max_degree <= 0 || mixing_parameter < 0 || mixing_parameter2 < 0 ||
        (nmax <= 0 && nmax != unlikely) || (nmin <= 0 && nmin != unlikely)) {
        cerr << "\n***********************\nERROR:\tsome positive parameters are negative" << endl;
        return -1;
    }
...
void statement() {
    ...
    cout
            << "Use option -sup (-inf) if you want to produce a benchmark whose distribution of the ratio of external in-degree/total in-degree ";
