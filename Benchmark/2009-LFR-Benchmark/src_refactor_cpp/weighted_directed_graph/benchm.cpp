...
int print_network(deque<set<int> > &Ein, deque<set<int> > &Eout, const deque<deque<int> > &member_list,
                  ...
                  deque<int> &num_seq, deque<map<int, double> > &neigh_weigh_in,
                  deque<map<int, double> > &neigh_weigh_out, double beta, double mu, double mu0) {
    int edges = 0;
    ...
    statout << endl << "--------------------------------------" << endl;
    cout << "average weight of an internal link " << average_func(inwij) << " +/- " << sqrt(variance_func(inwij))
         << endl;
    cout << "average weight of an external link " << average_func(outwij) << " +/- " << sqrt(variance_func(outwij))
         << endl;
...
int check_weights(deque<map<int, double> > &neigh_weigh_in, deque<map<int, double> > &neigh_weigh_out,
    ...
    for (int k = 0; k < neigh_weigh_in.size(); k++) {
        ...
        if (fabs(in_s - factual[k][0]) > 1e-7)
            cerr << in_s - factual[k][0];
        //cout<<"ok1"<<endl;
        if (fabs(out_s - factual[k][1]) > 1e-7)
            cerr << out_s - factual[k][1];
        //cout<<"ok2"<<endl;
        if (fabs(in_s + out_s + factual[k][2] - strs[k]) > 1e-7)
            cerr << in_s + out_s + factual[k][2] - strs[k];
        //cout<<"ok3"<<endl;
...
//*/
int
propagate_one(deque<map<int, double> > &neighbors_weights, const deque<int> &VE, const deque<deque<int> > &member_list,
              deque<deque<double> > &wished, deque<deque<double> > &factual, int i, double &tot_var, double *strs,
              const deque<int> &internal_kin_top, deque<map<int, double> > &others) {
...
int
propagate_two(deque<map<int, double> > &neighbors_weights, const deque<int> &VE, const deque<deque<int> > &member_list,
              deque<deque<double> > &wished, deque<deque<double> > &factual, int i, double &tot_var, double *strs,
              const deque<int> &internal_kin_top, deque<map<int, double> > &others) {
...
int propagate(deque<map<int, double> > &neigh_weigh_in, deque<map<int, double> > &neigh_weigh_out, const deque<int> &VE,
              const deque<deque<int> > &member_list,
              deque<deque<double> > &wished, deque<deque<double> > &factual, int i, double &tot_var, double *strs,
              const deque<int> &internal_kin_top) {
    propagate_one(neigh_weigh_in, VE, member_list, wished, factual, i, tot_var, strs, internal_kin_top,
                  neigh_weigh_out);
    propagate_one(neigh_weigh_out, VE, member_list, wished, factual, i, tot_var, strs, internal_kin_top,
                  neigh_weigh_in);
    propagate_two(neigh_weigh_in, VE, member_list, wished, factual, i, tot_var, strs, internal_kin_top,
                  neigh_weigh_out);
    propagate_two(neigh_weigh_out, VE, member_list, wished, factual, i, tot_var, strs, internal_kin_top,
                  neigh_weigh_in);
...
int weights(deque<set<int> > &ein, deque<set<int> > &eout, const deque<deque<int> > &member_list, const double beta,
    ...
    while (true) {
        ...
        for (int i = 0; i < VE.size(); i++)
            propagate(neigh_weigh_in, neigh_weigh_out, VE, member_list, wished, factual, i, tot_var, strs,
                      internal_kin_top);
        //check_weights(neigh_weigh_in, neigh_weigh_out, member_list, wished, factual, tot_var, strs);
...
int benchmark(bool excess, bool defect, int num_nodes, double average_k, int max_degree, double tau, double tau2,
              double mixing_parameter, double mixing_parameter2, double beta, int overlapping_nodes,
    ...
    print_network(Ein, Eout, member_list, member_matrix, num_seq, neigh_weigh_in, neigh_weigh_out, beta,
                  mixing_parameter2, mixing_parameter);
    return 0;
}
...
int main(int argc, char *argv[]) {
    ...
    benchmark(p.excess, p.defect, p.num_nodes, p.average_k, p.max_degree, p.tau, p.tau2, p.mixing_parameter,
              p.mixing_parameter2, p.beta, p.overlapping_nodes, p.overlap_membership, p.nmin, p.nmax, p.fixed_range);
    return 0;
}
