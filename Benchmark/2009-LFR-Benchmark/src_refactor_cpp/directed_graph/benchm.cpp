...
int internal_degree_and_membership(double mixing_parameter, int overlapping_nodes, int max_mem_num, int num_nodes,
                                   ...
                                   bool excess, bool defect, deque<int> &degree_seq_in, deque<int> &degree_seq_out,
    ...
    for (int i = 0; i < degree_seq_in.size(); i++) {
        ...
        if (ran4() < (interno - int_interno))
            int_interno++;
        if (excess) {
            while ((double(int_interno) / degree_seq_in[i] < (1 - mixing_parameter)) &&
                   (int_interno < degree_seq_in[i]))
                int_interno++;
        }
        if (defect) {
    ...
    for (int i = 0; i < degree_seq_out.size(); i++) {
        ...
        if (ran4() < (interno - int_interno))
            int_interno++;
        if (excess) {
            while ((double(int_interno) / degree_seq_out[i] < (1 - mixing_parameter)) &&
                   (int_interno < degree_seq_out[i]))
                int_interno++;
        }
        if (defect) {
    ...
    if (num_seq.empty()) {
        ...
        if (!fixed_range && (max_degree_actual + 1) > nmin) {
            _num_ = max_degree_actual +
                    1;            // this helps the assignment of the memberships (it assures that at least one module is big enough to host each node)
        ...
        while (internal_degree_seq_in[i] > available[available_nodes[try_this]]) {
            ...
            if (kr == 3 * num_nodes) {
                ...
                for (int i = 0; i < num_seq.size(); i++)
                    cout << num_seq[i] << " ";
                cout << endl << endl;
                return (internal_degree_and_membership(mixing_parameter, overlapping_nodes, max_mem_num, num_nodes,
                                                       member_matrix, excess, defect, degree_seq_in, degree_seq_out,
                                                       num_seq, internal_degree_seq_in, internal_degree_seq_out,
                                                       fixed_range, nmin, nmax, tau2));
            }
        }
...
*/
int build_subgraph(deque<set<int> > &Ein, deque<set<int> > &Eout, const deque<int> &nodes, const deque<int> &d_in,
                   const deque<int> &d_out) {
    /*
    cout<<"nodes"<<endl;
    prints(nodes);
        ...
        while (stop++ < stopper) {
            while (true) {
                ...
                if (random_mate == node || en_in[node].find(random_mate) != en_in[node].end())
                    break;
                deque<int> not_common;
                for (set<int>::iterator it_est = en_out[random_mate].begin();
                     it_est != en_out[random_mate].end(); it_est++)
    ...
    for (int i = 0; i < en_in.size(); i++) {
        for (set<int>::iterator its = en_in[i].begin(); its != en_in[i].end(); its++) {
            bool already = !(Ein[nodes[i]].insert(
                    nodes[*its]).second);        // true is the insertion didn't take place
            if (already)
        ...
        while (true) {
            ...
            if (Ein[a].find(random_mate) == Ein[a].end()) {
                ...
                for (set<int>::iterator it_est = Eout[random_mate].begin(); it_est != Eout[random_mate].end(); it_est++)
                    if ((b != (*it_est)) && (Eout[b].find(*it_est) == Eout[b].end()) &&
...
int build_subgraphs(deque<set<int> > &Ein, deque<set<int> > &Eout, const deque<deque<int> > &member_matrix,
                    ...
                    deque<deque<int> > &link_list_out,
                    const deque<int> &internal_degree_seq_in, const deque<int> &degree_seq_in,
                    const deque<int> &internal_degree_seq_out, const deque<int> &degree_seq_out, const bool excess,
                    const bool defect) {
    Ein.clear();
    Eout.clear();
    member_list.clear();
    ...
    */
    ...
    for (int i = 0; i < member_matrix.size(); i++) {
        ...
        for (int j = 0; j < member_matrix[i].size(); j++) {
            int right_index =
                    lower_bound(member_list[member_matrix[i][j]].begin(), member_list[member_matrix[i][j]].end(), i) -
        ...
        for (int diffloop = 0; diffloop < 3 * initial_diff; diffloop++) {
            ...
            for (int j = 0; j < member_matrix[i].size(); j++) {
                ...
                int right_index = lower_bound(member_list[random_mate].begin(), member_list[random_mate].end(), i) -
                ...
                if (internal_cluster_in > internal_cluster_out) {
                    if ((link_list_out[random_mate][right_index] < member_matrix[i].size() - 1) &&
                        (link_list_out[random_mate][link_list_out[random_mate].size() - 1] > 0)) {
...
int compute_var_mate(deque<set<int> > &en_in, const deque<deque<int> > &member_list) {
    int var_mate = 0;
    for (int i = 0; i < en_in.size(); i++)
        for (set<int>::iterator itss = en_in[i].begin(); itss != en_in[i].end(); itss++)
    ...
    */
    ...
    while (var_mate > 0) {
        ...
        for (int a = 0; a < d_in.size(); a++)
            for (set<int>::iterator its = en_in[a].begin(); its != en_in[a].end(); its++)
                if (they_are_mate(a, *its, member_list)) {
                    ...
                    while (true) {
                        ...
                        if (!(they_are_mate(a, random_mate, member_list)) &&
                            (en_in[a].find(random_mate) == en_in[a].end())) {
                            deque<int> not_common;
                            for (set<int>::iterator it_est = en_out[random_mate].begin();
                                 it_est != en_out[random_mate].end(); it_est++)
...
int internal_kin(deque<set<int> > &Ein, const deque<deque<int> > &member_list, int i) {
    int var_mate2 = 0;
    for (set<int>::iterator itss = Ein[i].begin(); itss != Ein[i].end(); itss++)
        if (they_are_mate(i, *itss, member_list))
            var_mate2++;
    return var_mate2;
}

int internal_kin_only_one(set<int> &Ein,
                          const deque<int> &member_matrix_j) {        // return the overlap between Ein and member_matrix_j
    int var_mate2 = 0;
...
int erase_links(deque<set<int> > &Ein, deque<set<int> > &Eout, const deque<deque<int> > &member_list, const bool excess,
    ...
    if (excess) {
        for (int i = 0; i < num_nodes; i++) {
            while ((Ein[i].size() > 1) &&
                   double(internal_kin(Ein, member_list, i)) / Ein[i].size() < 1 - mixing_parameter) {
                //---------------------------------------------------------------------------------
                ...
                if (deqar.size() == Ein[i].size()) {    // this shouldn't happen...
                    cerr
                            << "sorry, something went wrong: there is a node which does not respect the constraints. (option -sup)"
                            << endl;
                    return -1;
                }
    ...
    if (defect) {
        for (int i = 0; i < num_nodes; i++)
            while ((Ein[i].size() < Ein.size()) &&
                ...
                while (((they_are_mate(i, random_mate, member_list)) || Ein[i].find(random_mate) != Ein[i].end()) &&
                ...
                if (stopper_ == stopper_here) {    // this shouldn't happen...
                    cerr
                            << "sorry, something went wrong: there is a node which does not respect the constraints. (option -inf)"
                            << endl;
                    return -1;
                }
...
int print_network(deque<set<int> > &Ein, deque<set<int> > &Eout, const deque<deque<int> > &member_list,
    ...
    for (int i = 0; i < member_matrix.size(); i++) {
        ...
        for (int j = 0; j < member_matrix[i].size(); j++) {
            ...
            media_int += kinj;
            media_est += Ein[member_matrix[i][j]].size() -
                         double(internal_kin_only_one(Ein[member_matrix[i][j]], member_matrix[i]));
        }
    ...
    cout << "\n\n---------------------------------------------------------------------------" << endl;
    cout << "network of " << num_nodes << " vertices and " << edges << " edges" << ";\t average degree = "
         << double(edges) / num_nodes << endl;
    cout << "\naverage mixing parameter (in-links): " << average_func(double_mixing_in) << " +/- "
         << sqrt(variance_func(double_mixing_in)) << endl;
    cout << "average mixing parameter (out-links): " << average_func(double_mixing_out) << " +/- "
         << sqrt(variance_func(double_mixing_out)) << endl;
...
int benchmark(bool excess, bool defect, int num_nodes, double average_k, int max_degree, double tau, double tau2,
              double mixing_parameter, int overlapping_nodes, int overlap_membership, int nmin, int nmax,
              bool fixed_range) {
    ...
    if (internal_degree_and_membership(mixing_parameter, overlapping_nodes, overlap_membership, num_nodes,
                                       member_matrix, excess, defect, degree_seq_in, degree_seq_out, num_seq,
                                       internal_degree_seq_in, internal_degree_seq_out, fixed_range, nmin, nmax,
                                       tau2) == -1)
        return -1;
    ...
    if (build_subgraphs(Ein, Eout, member_matrix, member_list, link_list_in, link_list_out, internal_degree_seq_in,
                        degree_seq_in, internal_degree_seq_out, degree_seq_out, excess, defect) == -1)
        return -1;
