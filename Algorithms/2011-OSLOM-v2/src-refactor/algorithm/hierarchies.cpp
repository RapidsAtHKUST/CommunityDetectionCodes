...
void external_program_to_call(string network_file, oslom_net_global &matteo,
    ...
    for (auto ei = 0; ei < paras.to_run.size(); ei++) {
...
bool write_tp_of_this_level(int level, oslom_net_global &luca, char *directory_char, int original_dim) {
    ...
    if (level == 0)
        ...
        luca.get_covers(tps, soft_partitions_written,
                        paras.hier_gather_runs);        // here we get covers, which are written in file tps -without homeless-
    }
    /*********   GETTING COVERS   ***********/
    /*********   extrenal programs ***********/
    if (level == 0) {
    ...
    /*********   extrenal programs ***********/
    luca.ultimate_cover(tps, soft_partitions_written,
                        tp_ultimate);                    //here we get the final cover, which is written in file tp_(level) -with homeless-
    if (level == 0) {
...
void oslom_level(oslom_net_global &luca, char *directory_char) {
    ...
    int original_dim = 0;
    while (true) {
        cout << "network:: " << luca.size() << " nodes and " << luca.stubs() << " stubs;\t average degree = "
             << luca.stubs() / luca.size() << endl;
        if (level == 0)
