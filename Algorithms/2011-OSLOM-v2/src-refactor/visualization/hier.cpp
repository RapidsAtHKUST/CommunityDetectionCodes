...
int another_level(string short_tpn, string netn, string tpn, map<int, double> &lab_x, map<int, double> &lab_y,
                  ...
                  map<int, double> &lab_x_next, map<int, double> &lab_y_next, map<int, double> &lab_r_next, int level,
                  char *b1, deque<string> &edge_append) {
    //cherr();
    lab_x_next.clear();
    lab_y_next.clear();
    lab_r_next.clear();
    ...
    for (map<int, deque<int> >::iterator itm_M = Mlab.begin(); itm_M != Mlab.end(); itm_M++) {
        ...
        o_level(lab_x[itm_M->first], lab_y[itm_M->first], lab_r[itm_M->first], giovanni, sizes_h, lab_x_h, lab_y_h,
                lab_r_h);
...
int all_levels(int levels, string network_file) {
    ...
    for (int i = levels - 1; i >= 0; i--) {
        ...
        another_level(string(b4), string(b3), string(b2), lab_x, lab_y, lab_r, lab_x_next, lab_y_next, lab_r_next, i,
                      b1, edge_append);
        lab_x = lab_x_next;
        lab_y = lab_y_next;
        lab_r = lab_r_next;
