...
double visual_network::distance_node(int node1) {
    double distance = 0;
...
double visual_network::draw_pajek(deque<double> &angles, deque<double> &radii, double radius, double origin_x,
                                  double origin_y) {
    ofstream subout("pajek_file");
    subout << "*Vertices " << dim << endl;
    for (int i = 0; i < dim; i++)
        subout << "   " << i + 1 << " \"" << id_of(i) << "\" " << radius * cos(angles[i]) + origin_x << " "
               << radius * sin(angles[i]) + origin_y << " " << radii[i] << " " << " box ic " << "red" << endl;
    subout << "*Edges" << endl;
    for (int i = 0; i < dim; i++) {
...
double visual_network::draw_pajek(map<int, double> &lab_x, map<int, double> &lab_y, map<int, double> &lab_r) {
    ...
    subout << "*Vertices " << dim << endl;
    for (int i = 0; i < dim; i++)
        subout << "   " << i + 1 << " \"" << id_of(i) << "\" " << labx_h[i] << " " << laby_h[i] << " " << labr_h[i]
               << " " << " box ic " << "red" << endl;
    subout << "*Edges" << endl;
    for (int i = 0; i < dim; i++) {
        for (int j = 0; j < vertices[i]->links->size(); j++)
    ...
    for (int ii = 0; ii < group.size(); ii++) {
        ...
        for (int j = 0; j < vertices[nodei]->links->size(); j++) {
            ...
            if (binary_search(group.begin(), group.end(), vertices[nodei]->links->l[j]) &&
                nodei < vertices[nodei]->links->l[j])
...
double visual_network::draw_gnuplot(map<int, double> &lab_x, map<int, double> &lab_y, map<int, double> &lab_r,
                                    deque<string> &edge_append, multimap<int, int> &internal_links) {
    map<int, double> labx_h;
    map<int, double> laby_h;
    map<int, double> labr_h;
...
double visual_network::draw_gnuplot(map<int, double> &lab_x, map<int, double> &lab_y, map<int, double> &lab_r,
                                    string position_file, string gfile) {
    map<int, double> labx_h;
    map<int, double> laby_h;
    map<int, double> labr_h;
    relabel(lab_x, lab_y, lab_r, labx_h, laby_h, labr_h);
...
double visual_network::relabel(map<int, double> &lab_x, map<int, double> &lab_y, map<int, double> &lab_r,
                               map<int, double> &labx_h, map<int, double> &laby_h, map<int, double> &labr_h) {
    labx_h.clear();
    laby_h.clear();
    labr_h.clear();
    map<int, int> A;
...
double visual_network::with_colors(deque<deque<int> > &ten) {
    ...
    subout << "*Vertices " << dim << endl;
    for (int i = 0; i < dim; i++)
        subout << "   " << i + 1 << " \"" << id_of(i) << "\" " << (0.7 * cos(theta * pos[i]) + 1) / 2.1 << " "
               << (0.7 * sin(theta * pos[i]) + 1) / 2.1 << " 0.5" << " box ic "
               << pajek_colors[mem[i] % pajek_colors.size()] << endl;
    subout << "*Edges" << endl;
    for (int i = 0; i < dim; i++) {
        for (int j = 0; j < vertices[i]->links->size(); j++)
            if (i < vertices[i]->links->l[j])
...
int visual_network::circles(int secs, map<int, int> &lab_sizes, deque<double> &angles, deque<double> &radii,
                            double radius) {
