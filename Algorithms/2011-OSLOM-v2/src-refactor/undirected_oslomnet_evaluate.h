...
public:
    ...
    oslomnet_evaluate(deque <deque<int>> &b, deque <deque<pair < int, double>>
    ...
    oslomnet_evaluate(map<int, map<int, pair < int, double> >
...
private:
    ...
    double
    all_external_test(int kout_g, int tm, int Nstar, int nneighs, const double &max_r_one, const double &maxr_two,
                      deque<int> &gr_cleaned, bool only_c, weighted_tabdeg &previous_tab_c);

    double cup_on_list(cup_data_struct &a, deque<int> &gr_cleaned);

    void
    get_external_scores(weighted_tabdeg &ne_, cup_data_struct &fitness_label_to_sort, int kout_g, int tm, int Nstar,
                        int nneighs, const double &max_r, bool only_c, weighted_tabdeg &previous_tab_c);


    double CUP_runs(weighted_tabdeg &previous_tab_c, weighted_tabdeg &previous_tab_n, int kin_cgroup_prev,
                    int ktot_cgroup_prev, deque<int> &gr_cleaned, bool only_c, int runs);
    ...
    double partial_CUP(weighted_tabdeg &previous_tab_c, weighted_tabdeg &previous_tab_n, int kin_cgroup_prev,
                       int ktot_cgroup_prev, deque<int> &gr_cleaned, bool only_c);
...
void oslomnet_evaluate::erase_cgroup(int wnode) {
    ...
    if (itm != cgroup.lab_facts.end()) {
        ...
        for (int i = 0; i < vertices[wnode]->links->size(); i++) {
            ...
            if (cgroup.update_group(vertices[wnode]->links->l[i], -vertices[wnode]->links->w[i].first,
                                    -vertices[wnode]->links->w[i].second,
                                    dim - cgroup.size(), neighs.size(), kout_g, tm,
                                    vertices[vertices[wnode]->links->l[i]]->stub_number, tobe) == false)
                neighs.update_neighs(vertices[wnode]->links->l[i], -vertices[wnode]->links->w[i].first,
                                     -vertices[wnode]->links->w[i].second,
...
double oslomnet_evaluate::all_external_test(int kout_g, int tm, int Nstar, int nneighs, const double &max_r_one,
                                            const double &maxr_two, deque<int> &gr_cleaned, bool only_c,
...
oslomnet_evaluate::partial_CUP(weighted_tabdeg &previous_tab_c, weighted_tabdeg &previous_tab_n, int kin_cgroup_prev,
                               int ktot_cgroup_prev, deque<int> &border_group, bool only_c) {
    ...
    while (true) {
        ...
        bscore = all_external_test(ktot_cgroup - kin_cgroup, oneM - ktot_cgroup, dim - cgroup.size(), neighs.size(),
                                   maxb_nodes / double(dim - cgroup.size()), max_r_bord, border_group, only_c,
                                   previous_tab_c);
...
oslomnet_evaluate::CUP_runs(weighted_tabdeg &previous_tab_c, weighted_tabdeg &previous_tab_n, int kin_cgroup_prev,
                            int ktot_cgroup_prev, deque<int> &gr_cleaned, bool only_c, int number_of_runs) {
    ...
    for (int i = 0; i < number_of_runs; i++) {
        ...
        double score_i = partial_CUP(previous_tab_c, previous_tab_n, kin_cgroup_prev, ktot_cgroup_prev, gr_run_i,
                                     only_c);
...
oslomnet_evaluate::CUP_check(const deque<int> &_c_, deque<int> &gr_cleaned, int number_of_runs = paras.clean_up_runs) {
    ...
    bscore = CUP_runs(previous_tab_c, previous_tab_n, kin_cgroup_prev, ktot_cgroup_prev, gr_cleaned, true,
                      number_of_runs);
...
oslomnet_evaluate::CUP_search(const deque<int> &_c_, deque<int> &gr_cleaned, int number_of_runs = paras.clean_up_runs) {
    ...
    initialize_for_evaluation(_c_, previous_tab_c, previous_tab_n, kin_cgroup_prev, ktot_cgroup_prev);
    bscore = CUP_runs(previous_tab_c, previous_tab_n, kin_cgroup_prev, ktot_cgroup_prev, gr_cleaned, false,
                      number_of_runs);
...
void oslomnet_evaluate::get_external_scores(weighted_tabdeg &neighs, cup_data_struct &fitness_label_to_sort, int kout_g,
                                            int tm, int Nstar, int nneighs, const double &max_r, bool only_c,
                                            weighted_tabdeg &previous_tab_c) {
    ...
    while (bit != neighs.fitness_lab.end()) {
        ...
        double interval;
        double F = compute_global_fitness(itm->second.internal_degree, kout_g, tm, itm->second.degree,
                                          itm->second.minus_log_total_wr, nneighs, Nstar, interval);
