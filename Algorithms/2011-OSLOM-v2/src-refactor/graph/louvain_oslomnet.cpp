...
void oslomnet_louvain::unweighted_favorite_of(const int &node, int &fi, int &kp, int &kop) {
    ...
    for (map<int, int>::iterator itM = M.begin(); itM != M.end(); itM++) {
        ...
        if (itM->first != vertex_label[node]) {
            to_fit = topological_05(itM->second, itOM->second.kout, oneM - itOM->second.ktot,
                                    vertices[node]->stub_number);
        } else {
            kop = itM->second;
            int kout_prime = itOM->second.kout - vertices[node]->stub_number + 2 * kop;
            to_fit = topological_05(itM->second, kout_prime, oneM - itOM->second.ktot + vertices[node]->stub_number,
                                    vertices[node]->stub_number);
...
void oslomnet_louvain::weighted_favorite_of(const int &node, int &fi, int &kp, int &kop) {
    ...
    for (mapip::iterator itM = M.begin(); itM != M.end(); itM++) {
        ...
        double to_fit;
        if (itM->first != vertex_label[node]) {
            to_fit = topological_05(itM->second.first, itOM->second.kout, oneM - itOM->second.ktot,
                                    vertices[node]->stub_number);
            to_fit *= double(dim - itOM->second.nc + 1) /
                      (min(dim - itOM->second.nc, itOM->second.kout / itM->second.first + 1) + 1);
            if (to_fit > 1)
                to_fit = 1;
        } else {
            kop = itM->second.first;
            int kout_prime = itOM->second.kout - vertices[node]->stub_number + 2 * kop;
            to_fit = topological_05(itM->second.first, kout_prime,
                                    oneM - itOM->second.ktot + vertices[node]->stub_number,
                                    vertices[node]->stub_number);
