...
void oslomnet_evaluate::erase_cgroup(int wnode) {
    ...
    if (itm != cgroup.lab_facts.end()) {
        ...
        double fi = compute_global_fitness_ofive(kpin, kout_g_in, kpout, kout_g_out, tmin, tmout, ktin, ktout, mtlwin,
                                                 mtlwout, neighs.size() + 1, dim - cgroup.size() + 1);
        ...
        for (int i = 0; i < vertices[wnode]->inlinks->size(); i++) {
            ...
            if (cgroup.update_group(vertices[wnode]->inlinks->l[i], -vertices[wnode]->inlinks->w[i].first,
                                    -OPA.second.first, -vertices[wnode]->inlinks->w[i].second, -OPA.second.second,
                                    dim - cgroup.size(), neighs.size(), kout_g_in, kout_g_out, tmin, tmout,
                                    vertices[vertices[wnode]->inlinks->l[i]]->instub_number,
                                    vertices[vertices[wnode]->inlinks->l[i]]->outstub_number, tobe) == false)
                neighs.update_neighs(vertices[wnode]->inlinks->l[i], -vertices[wnode]->inlinks->w[i].first,
                                     -OPA.second.first, -vertices[wnode]->inlinks->w[i].second, -OPA.second.second,
                                     dim - cgroup.size(), kout_g_in, kout_g_out, tmin, tmout,
                                     vertices[vertices[wnode]->inlinks->l[i]]->instub_number,
        ...
        for (int i = 0; i < vertices[wnode]->outlinks->size(); i++) {
            ...
            if (OPA.first == -1)
                if (cgroup.update_group(vertices[wnode]->outlinks->l[i], 0, -vertices[wnode]->outlinks->w[i].first, 0,
                                        -vertices[wnode]->outlinks->w[i].second,
                                        dim - cgroup.size(), neighs.size(), kout_g_in, kout_g_out, tmin, tmout,
                                        vertices[vertices[wnode]->outlinks->l[i]]->instub_number,
                                        vertices[vertices[wnode]->outlinks->l[i]]->outstub_number, tobe) == false)
                    neighs.update_neighs(vertices[wnode]->outlinks->l[i], 0, -vertices[wnode]->outlinks->w[i].first, 0,
                                         -vertices[wnode]->outlinks->w[i].second,
                                         dim - cgroup.size(), kout_g_in, kout_g_out, tmin, tmout,
                                         vertices[vertices[wnode]->outlinks->l[i]]->instub_number,
...
void oslomnet_evaluate::insert_cgroup(int wnode) {
    ...
    int tmout = oneM - ktot_cgroup_out;
    double fi = compute_global_fitness_ofive(kpin, kout_g_in, kpout, kout_g_out, tmin, tmout, ktin, ktout, mtlwin,
                                             mtlwout, neighs.size(), dim - cgroup.size());
    kin_cgroup += kpin + kpout;
    ktot_cgroup_in += ktin;
    ktot_cgroup_out += ktout;
    ...
    for (int i = 0; i < vertices[wnode]->inlinks->size(); i++) {
        ...
        if (cgroup.update_group(vertices[wnode]->inlinks->l[i], vertices[wnode]->inlinks->w[i].first, OPA.second.first,
                                vertices[wnode]->inlinks->w[i].second, OPA.second.second,
                                dim - cgroup.size(), neighs.size(), kout_g_in, kout_g_out, tmin, tmout,
                                vertices[vertices[wnode]->inlinks->l[i]]->instub_number,
                                vertices[vertices[wnode]->inlinks->l[i]]->outstub_number, tobe) == false)
            neighs.update_neighs(vertices[wnode]->inlinks->l[i], vertices[wnode]->inlinks->w[i].first, OPA.second.first,
                                 vertices[wnode]->inlinks->w[i].second, OPA.second.second,
                                 dim - cgroup.size(), kout_g_in, kout_g_out, tmin, tmout,
                                 vertices[vertices[wnode]->inlinks->l[i]]->instub_number,
                                 vertices[vertices[wnode]->inlinks->l[i]]->outstub_number);
        ...
        if (c_pos < critical_xi) {
            ...
            */
            if (until == -1) {            // this node is the first node to be below the threshold
                ...
                */
                double probability_to_overtake = compare_r_variables(probability_a, probability_b,
                                                                     itl->first - itl->second.second,
                                                                     itl->first + itl->second.second);
                //cout<<"probability_to_overtake: "<<probability_to_overtake<<endl;
                if (probability_to_overtake >
                    0.4999) {        /*preliminary check: this node is basically equivalent to the previous guy, I consider it good*/
                    until = Nstar - pos;
                    c_min = c_pos;
                    probability_a = itl->first - itl->second.second;
                    probability_b = itl->first + itl->second.second;
                    ...
                    if ((probability_to_overtake == 0) || ((1. - probability_to_overtake) *
                                                           compute_probability_to_stop(probability_a, probability_b,
                                                                                       critical_xi, Nstar, pos + 1) >
                                                           0.5001)) {
            ...
            if (until !=
                -1) {        /* this means that this node is not good and the previous one was good. So, I stop here */
                if (equivalent_check_gather(a, until, probability_a, probability_b, Nstar, critical_xi))
                    break;
            }
        }
...
double oslomnet_evaluate::all_external_test(int kout_g_in, int tmin, int kout_g_out, int tmout,
    ...
    get_external_scores(neighs, fitness_label_to_sort, kout_g_in, tmin, kout_g_out, tmout, Nstar, nneighs, max_r,
                        only_c, previous_tab_c);
    return cup_on_list(fitness_label_to_sort, gr_cleaned);
}

void oslomnet_evaluate::initialize_for_evaluation(weighted_tabdeg &previous_tab_c, weighted_tabdeg &previous_tab_n,
...
void oslomnet_evaluate::initialize_for_evaluation(const deque<int> &_c_, weighted_tabdeg &previous_tab_c,
                                                  weighted_tabdeg &previous_tab_n, int &kin_cgroup_prev,
                                                  int &ktot_cgroup_prev_in, int &ktot_cgroup_prev_out) {
...
double
oslomnet_evaluate::partial_CUP(weighted_tabdeg &previous_tab_c, weighted_tabdeg &previous_tab_n, int kin_cgroup_prev,
                               int ktot_cgroup_prev_in, int ktot_cgroup_prev_out, deque<int> &border_group,
                               bool only_c) {
    ...
    double bscore = 1;
    while (true) {
        bscore = all_external_test(ktot_cgroup_in - kin_cgroup, oneM - ktot_cgroup_in, ktot_cgroup_out - kin_cgroup,
                                   oneM - ktot_cgroup_out,
                                   dim - cgroup.size(), neighs.size(), maxb_nodes / double(dim - cgroup.size()),
                                   max_r_bord, border_group, only_c, previous_tab_c);
...
double
oslomnet_evaluate::CUP_runs(weighted_tabdeg &previous_tab_c, weighted_tabdeg &previous_tab_n, int kin_cgroup_prev,
                            int ktot_cgroup_prev_in, int ktot_cgroup_prev_out,
    ...
    for (int i = 0; i < number_of_runs; i++) {
        deque<int> gr_run_i;
        double score_i = partial_CUP(previous_tab_c, previous_tab_n, kin_cgroup_prev, ktot_cgroup_prev_in,
                                     ktot_cgroup_prev_out, gr_run_i, only_c);
...
double
oslomnet_evaluate::CUP_check(const deque<int> &_c_, deque<int> &gr_cleaned, int number_of_runs = paras.clean_up_runs) {
    ...
    double bscore;
    initialize_for_evaluation(_c_, previous_tab_c, previous_tab_n, kin_cgroup_prev, ktot_cgroup_prev_in,
                              ktot_cgroup_prev_out);
    bscore = CUP_runs(previous_tab_c, previous_tab_n, kin_cgroup_prev, ktot_cgroup_prev_in, ktot_cgroup_prev_out,
                      gr_cleaned, true, number_of_runs);
    return bscore;
}

double
oslomnet_evaluate::CUP_search(const deque<int> &_c_, deque<int> &gr_cleaned, int number_of_runs = paras.clean_up_runs) {
    /*_c_ is the module to clean up and gr_cleaned is the result */
    weighted_tabdeg previous_tab_c;
    weighted_tabdeg previous_tab_n;
    int kin_cgroup_prev;
...
double
oslomnet_evaluate::CUP_both(const deque<int> &_c_, deque<int> &gr_cleaned, int number_of_runs = paras.clean_up_runs) {
    /*_c_ is the module to clean up and gr_cleaned is the result */
    weighted_tabdeg previous_tab_c;
    weighted_tabdeg previous_tab_n;
    int kin_cgroup_prev;
    ...
    bscore = CUP_runs(previous_tab_c, previous_tab_n, kin_cgroup_prev, ktot_cgroup_prev_in, ktot_cgroup_prev_out,
                      gr_cleaned, true,
                      number_of_runs);        /*this "true" means I can only look at nodes in previous_tab_c*/
    return bscore;
}

double oslomnet_evaluate::CUP_iterative(const deque<int> &_c_, deque<int> &gr_cleaned,
...
double oslomnet_evaluate::group_inflation(const deque<int> &_c_, deque<int> &gr_cleaned,
                                          int number_of_runs = paras.inflate_runs) {
    /* preliminary check */
    ...
    while (true) {
        ...
        initialize_for_evaluation(previous_tab_c, previous_tab_n, kin_cgroup_prev, ktot_cgroup_prev_in,
                                  ktot_cgroup_prev_out);
        CUP_runs(previous_tab_c, previous_tab_n, kin_cgroup_prev, ktot_cgroup_prev_in, ktot_cgroup_prev_out, gr_cleaned,
                 false, number_of_runs);
        if (gr_cleaned.size() > 0) {
            /*the first clean up passed. now it makes the CUP_check*/
            initialize_for_evaluation(gr_cleaned, previous_tab_c, previous_tab_n, kin_cgroup_prev, ktot_cgroup_prev_in,
                                      ktot_cgroup_prev_out);
            bscore = CUP_runs(previous_tab_c, previous_tab_n, kin_cgroup_prev, ktot_cgroup_prev_in,
                              ktot_cgroup_prev_out, gr_cleaned, true, paras.clean_up_runs);
            if (gr_cleaned.size() > 0) {
                //cout<<"test passed, bscore: "<<bscore<<"  size: "<<gr_cleaned.size()<<endl;
                return bscore;
            }
...
void
oslomnet_evaluate::get_external_scores(weighted_tabdeg &neighs, cup_data_struct &fitness_label_to_sort, int kout_g_in,
                                       int tmin, int kout_g_out, int tmout,
                                       int Nstar, int nneighs, const double &max_r, bool only_c,
                                       weighted_tabdeg &previous_tab_c) {
    ...
    while (bit != neighs.fitness_lab.end()) {
        ...
        *-**************************************************************************************/
        double F = compute_global_fitness(itm->second.internal_indegree, kout_g_in, itm->second.internal_outdegree,
                                          kout_g_out, tmin, tmout,
                                          itm->second.indegree, itm->second.outdegree, itm->second.minus_log_total_wrin,
                                          itm->second.minus_log_total_wrout, nneighs, Nstar, interval);
...
int oslomnet_evaluate::try_to_assign_homeless_help(module_collection &module_coll, map<int, deque<int> > &to_check) {
    ...
    for (UI i = 0; i < homel.size(); i++) {
        ...
        for (int j = 0; j < vertices[homel[i]]->inlinks->size(); j++) {
            ...
            for (set<int>::iterator itk = module_coll.memberships[neigh].begin();
                 itk != module_coll.memberships[neigh].end(); itk++) {
                called.insert(*itk);
                thish.insert(*itk);
            }
        }
    ...
    for (map<int, set<int> >::iterator itm = homel_module.begin(); itm != homel_module.end(); itm++) {
        ...
        for (set<int>::iterator its = itm->second.begin(); its != itm->second.end(); its++) {
            ...
            double rh1 = compute_global_fitness_randomized_short(kin_node_in, kout_g_out, tmin,
                                                                 vertices[itm->first]->instub_number, kinw_in);
            double rh2 = compute_global_fitness_randomized_short(kin_node_out, kout_g_in, tmout,
                                                                 vertices[itm->first]->outstub_number, kinw_out);
