//
// Created by cheyulin on 3/13/17.
//
...
void static_network::set_graph(map<int, map<int, pair<int, double> > > &A) {
    ...
    for (map<int, map<int, pair<int, double> > >::iterator itm = A.begin(); itm != A.end();
         itm++) {
            ...
            for (int j = 0; j < vertices[i]->links->size(); j++)
                if (vertices[i]->id_num <= vertices[vertices[i]->links->l[j]]->id_num)
                    graph_out << vertices[i]->id_num << "\t" << vertices[vertices[i]->links->l[j]]->id_num << "\t"
                              << vertices[i]->original_weights[j] << "\t" << vertices[i]->links->w[j].first << "\t"
                              << endl;
    } else {
        for (int i = 0; i < int(vertices.size()); i++)
            for (int j = 0; j < vertices[i]->links->size(); j++)
                if (vertices[i]->id_num <= vertices[vertices[i]->links->l[j]]->id_num)
                    graph_out << vertices[i]->id_num << "\t" << vertices[vertices[i]->links->l[j]]->id_num << "\t"
                              << vertices[i]->links->w[j].first << endl;
    }
    return 0;
}
        ...
        for (int i = 0; i < dim; i++) {
            for (int j = 0; j < vertices[i]->links->size(); j++) {
                ...
                double w2 = (vertices[vertices[i]->links->l[j]]->strength /
                             vertices[vertices[i]->links->l[j]]->stub_number) * vertices[i]->links->w[j].first;
            ...
            for (int j = 0; j < vertices[i]->links->size(); j++)
                if (i < vertices[i]->links->l[j])
                    graph_out << i << "\t" << vertices[i]->links->l[j] << "\t"
                              << cast_int(vertices[i]->original_weights[j]) << endl;
    } else {
        for (int i = 0; i < int(vertices.size()); i++)
            for (int j = 0; j < vertices[i]->links->size(); j++)
                if (i < vertices[i]->links->l[j])
                    graph_out << i << "\t" << vertices[i]->links->l[j] << "\t" << vertices[i]->links->w[j].first
                              << endl;
    }
    char bb[128];
...
int static_network::set_upper_network(map<int, map<int, pair<int, double> > > &neigh_weight_f,
    ...
    for (map<int, map<int, pair<double, double> > >::iterator itm = neigh_weight_s.begin(); itm !=
                                                                                            neigh_weight_s.end();
         itm++) {
        for (map<int, pair<double, double> >::iterator itm_ = itm->second.begin(); itm_ != itm->second.end();
             itm_++)
