...
void static_network::set_graph(deque<deque<int> > &link_per_node, deque<deque<pair<int, double> > > &weights_per_node,
    ...
    for (auto i = 0; i < label_rows.size(); i++)
    ...
    for (auto i = 0; i < link_per_node.size(); i++)
        for (auto j = 0; j < link_per_node[i].size(); j++)
...
int static_network::draw_consecutive(string file_name1, string file_name2) {
    ...
    if (paras.weighted) {
        for (auto i = 0; i < vertices.size(); i++)
            for (int j = 0; j < vertices[i]->outlinks->size(); j++)
                graph_out << i << "\t" << vertices[i]->outlinks->l[j] << "\t"
                          << cast_int(vertices[i]->out_original_weights[j]) << endl;
    } else {
        for (auto i = 0; i < vertices.size(); i++)
            for (int j = 0; j < vertices[i]->outlinks->size(); j++)
...
int static_network::draw(string file_name) {
    ...
    if (paras.weighted) {
        for (auto i = 0; i < vertices.size(); i++)
            for (int j = 0; j < vertices[i]->outlinks->size(); j++)
                graph_out << vertices[i]->id_num << "\t" << vertices[vertices[i]->outlinks->l[j]]->id_num << "\t"
                          << vertices[i]->out_original_weights[j] << "\t" << vertices[i]->outlinks->w[j].first << endl;
    } else {
        for (auto i = 0; i < vertices.size(); i++)
            for (int j = 0; j < vertices[i]->outlinks->size(); j++)
                graph_out << vertices[i]->id_num << "\t" << vertices[vertices[i]->outlinks->l[j]]->id_num << "\t"
                          << vertices[i]->outlinks->w[j].first << endl;
...
int static_network::draw_with_weight_probability(string file_name) {
    ...
    if (paras.weighted) {
        for (auto i = 0; i < vertices.size(); i++)
            for (int j = 0; j < vertices[i]->outlinks->size(); j++)
                graph_out << vertices[i]->id_num << "\t" << vertices[vertices[i]->outlinks->l[j]]->id_num << "\t"
                          << vertices[i]->out_original_weights[j] << "\t" << vertices[i]->outlinks->w[j].first << " "
                          << vertices[i]->outlinks->w[j].second << endl;
    } else {
        for (auto i = 0; i < vertices.size(); i++)
            for (int j = 0; j < vertices[i]->outlinks->size(); j++)
                graph_out << vertices[i]->id_num << "\t" << vertices[vertices[i]->outlinks->l[j]]->id_num << "\t"
...
int static_network::translate(deque<deque<int> > &ten) {
    ...
    for (auto i = 0; i < ten.size(); i++) {
        deque<int> ff;
        for (auto j = 0; j < ten[i].size(); j++) {
    ...
    for (auto i = 0; i < group.size(); i++) {
        ...
        for (int j = 0; j < vertices[nodei]->outlinks->size(); j++)
            if (binary_search(group.begin(), group.end(), vertices[nodei]->outlinks->l[j])) {
                ...
                if (paras.weighted)
                    weight_i.push_back(
                            make_pair(vertices[nodei]->outlinks->w[j].first, vertices[nodei]->out_original_weights[j]));
                else
    ...
    if (dim == 0) {
        ...
        for (int i = 0; i < dim; i++) {
            for (int j = 0; j < vertices[i]->outlinks->size(); j++) {
                ...
                double w1 =
                        (vertices[i]->outstrength / vertices[i]->outstub_number) * vertices[i]->outlinks->w[j].first;
                double w2 = (vertices[neigh]->instrength / vertices[neigh]->instub_number) *
                            vertices[i]->outlinks->w[j].first;
...
int static_network::propagate_distances(deque<int> &new_shell, set<int> &already_gone,
    ...
    shell++;
    deque<int> next_shell;
    for (auto i = 0; i < new_shell.size(); i++) {
...
int static_network::set_upper_network(map<int, map<int, pair<int, double> > > &neigh_weight_f,
                                      module_collection &module_coll) {
    ...
    for (map<int, map<int, pair<double, double> > >::iterator itm = neigh_weight_s.begin();
         itm != neigh_weight_s.end(); itm++) {
...
void static_network::print_degree_of_homeless(DI &homel, ostream &outt) {
    ...
    outt << "average in-degree of homeless nodes: " << average_func(degree_of_homeless) << " dev: "
    ...
    outt << "average out-degree of homeless nodes: " << average_func(degree_of_homeless) << " dev: "
         << sqrt(variance_func(degree_of_homeless)) << endl;
    degree_of_homeless.clear();
    for (auto i = 0; i < homel.size(); i++)
        degree_of_homeless.push_back(vertices[homel[i]]->instub_number + vertices[homel[i]]->outstub_number);
    outt << "average in+out-degree of homeless nodes: " << average_func(degree_of_homeless) << " dev: "
         << sqrt(variance_func(degree_of_homeless)) << endl;
}
