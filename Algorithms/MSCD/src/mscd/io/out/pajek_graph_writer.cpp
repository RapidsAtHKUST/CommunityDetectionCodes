...
static bool registered = toolkit::Registry<GraphWriter>::GetInstance().Register(new PajekGraphWriter);
	
bool PajekGraphWriter::ToFile(const std::string & filename, const ds::Graph & g) {
	FILE * f = fopen(filename.c_str(), "w");
	if (!f) return false;
	const ds::Graph::adjlist & al = g.GetAdjList();
	fprintf(f, "*Vertices %d\n", static_cast<int>(al.size()));
	fprintf(f, "*Arcs\n");
	fprintf(f, "*Edges\n");
