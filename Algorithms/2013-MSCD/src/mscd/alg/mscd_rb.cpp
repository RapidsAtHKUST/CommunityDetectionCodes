 ...
 *  MSCD
 *
 *  Created by Erwan Le Martelot on 29/11/2011.
...
static bool registered = toolkit::Registry<MSCDAlgorithm>::GetInstance().Register(new MSCD_RB);

bool MSCD_RB::Run(const ds::Graph & g,
	...
	for (int p=0; p<ps.size(); ++p) {
		...
		Q = this->computeQ(tw, twi, i2m, ps[p]);
		...
		while (check_nodes) {
			...
			do {
				...
				while (!l.empty()) {
					...
					// Find neighbour communities of c and its connection strength to them
					nbc.clear();
					nbc_wn.clear();
...
#ifdef __DEBUG__
double MSCD_RB::DbCheckQ(const ds::Graph & g, const std::vector<long> & ncom, const double p) const {
	const ds::Graph::adjlist & al = g.GetAdjList();
	...
	for (long i=0, j; i<al.size(); ++i) {
		for (j=0; j<al.size(); ++j) {
			if (ncom[i] == ncom[j]) {
				...
				q2 += (d[i]*d[j])/g.GetTotalWeight();
			}
		}
	}
	return (q1 - p*q2)/g.GetTotalWeight();
}
#endif
...
// Alernative solution: using find (slow when community is large)
//*std::find(coms[c].begin(), coms[c].end(), n) = coms[c].back();

/*
 std::vector<long>::iterator it = std::find(coms[c].begin(), coms[c].end(), n);
 long di = std::distance(coms[c].begin(), it);
 if (di != ncidx[n]) {
 printf("!!! Di=%ld ncidx=%ld\n", di, ncidx[n]);
 }
*/