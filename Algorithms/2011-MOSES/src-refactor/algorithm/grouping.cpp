...
namespace grouping {
    ...
    Grouping::Grouping(const GraphWithoutNames &g, long double p_in, long double p_out)
            : _p_in(p_in), _p_out(p_out), _g(g), groups(_groups), _vgroups(g.vcount()),
              comm_count_per_edge2(_comm_count_per_edge2), _sigma_shared(0), value_of_objective_with_no_communities(
                    1) // This should never be positive. Hence 1 will signify that it's undefined
    ...
    void Grouping::isolate(V v) {
        set<Group *> grps = vgroups(
                v); // I'm copying it in here, as we'll be modifying the node's vgroup as we delete nodes.
    ...
    void Grouping::FixGlobalEdgeCounts(const V *edgeVN_ptr, bool AddNotDel) {
        ...
        assert(_comm_count_per_edge2[edgeVN_ptr - _g.neighbours(0).first].shared_comms ==
               _comm_count_per_edge2[edgeNV_offset].shared_comms);
    }
    ...
    void deleteEmptyGroup(Grouping *pging,
                          Group *grp) { // TODO: This is only called in one place, and shouldn't be called from outside. Merge this into delV
