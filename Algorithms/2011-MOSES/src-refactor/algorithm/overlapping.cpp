...
namespace overlapping {
    ...
    template<class N>
    static void overlappingT(bloomGraph<N> &g);
    ...
    static pair<long double, set<V> >
    growThisEdge(Grouping &ging, V edgeNumber, const long double &boost, bool randomized_p_in);
    ...
    template<class N>
    static void groupStats(const Grouping &ging, bloomGraph<N> &g);
...
*/
        ...
        if (option_loadOverlapping[0]) { // load  preexisting grouping
            ...
            while (inFile.peek() != EOF) {
                ...
                if (linestream.peek() == '"') {
                    ...
                    if (linestream.peek() != '\t')
                        Die("grouping file should have a tab after any \" \". line %d {%d '%c'}", lineNo, (int) ch, ch);
                }
        ...
        for (int k = 0; k < max_OuterIters; k++) {
            ...
            if (!randomize_p_in && /*num_groups_after > 1000 && */ 0.99L * num_groups_after <= num_groups_before) {
                Pn("breaking after %d growing passes, as VERY FEW more have been found among the most recent subset",
                   k + 1);
                break;
            }
        }
    ...
    static void groupStats(const Grouping &ging, bloomGraph<N> &g) {
        ...
        { // now, just the randomized ones
            size_t max_group_size =
                    group_sizes_of_the_randomized.size() == 0 ? 0 : group_sizes_of_the_randomized.rbegin()->first;
            int entries_per_row = 15;
            int number_of_rows = (max_group_size / entries_per_row) + 1;
            for (int r = 0; r < number_of_rows; r++) {
                for (size_t c = r; c <= max_group_size; c += number_of_rows) {
    ...
    struct DeltaSeed {
        ...
        long double deltadeltaPairEntropy() const {
            return _deltadeltaEdgeEntropy +
                   log2l(1.0L - _ging._p_in) * (_group_size_smaller - count_edges_back_into_this_group);
        }

        explicit DeltaSeed(V v, int group_size_smaller, Grouping &ging) : _v(v),
                                                                          _group_size_smaller(group_size_smaller),
                                                                          _ging(ging), _deltadeltaEdgeEntropy(0.0L),
                                                                          count_edges_back_into_this_group(0) {
        ...
        void redoEdge(V,
                      int previous_sharedCommunities) {
            ...
            _deltadeltaEdgeEntropy +=
                    log2l(1.0L - (1.0L - _ging._p_out) * powl(1.0L - _ging._p_in, 2 + previous_sharedCommunities))
                    - log2l(1.0L - (1.0L - _ging._p_out) * powl(1.0L - _ging._p_in, 1 + previous_sharedCommunities));
        }
        ...
        long double _deltaZentropy() const {
            ...
            return (x2 * log2l(x2) + (N - x2) * log2l(N - x2) - x * log2l(x) - (N - x) * log2l(N - x));
        }
    ...
    pair<long double, set<V> >
    growingSeed(Grouping &ging, int lookahead, set<V> &seed, pair<long double, set<V> > bestSoFar,
                long double seedEdgeEntropy, int seed_size, Frontier &frontier, int edges_in_seed,
...
*/
...
#endif
        const long double newseedEdgeEntropy =
                seedEdgeEntropy + frontier.best_node_score() + log2l(1.0L - ging._p_in) * seed_size;
        ...
        UNUSED       int64 q = ging.groups.size();
        if (q == 0) q = 1;
        ...
        if ((size_t) seed_size >= max_commsize) { // max comm size
            bestSoFar.first = -10000000;
            return bestSoFar;
        }
        //if( bestSoFar.first > 0.0L) // once positive, return immediately!!!!!!!!!!!!!
        //return bestSoFar;
        if ((size_t) seed_size > bestSoFar.second.size() &&
            bestSoFar.first + boost > 0.0L) // once positive, return immediately if it drops.
            return bestSoFar;
        ...
        Foreach(V n, ns) {
                    ...
                    if (0 == seed.count(n))
                        frontier.addNode(ging, n, edgeVN_offset
                        );
                    ++edgeVN_offset;
                }
        seed.insert(best_v);
        return growingSeed(ging, lookahead, seed, bestSoFar, newseedEdgeEntropy, 1 + seed_size, frontier, edges_in_seed,
                           boost, randomized_p_in
        );
    }
    ...
    bool emptyIntersection(const pair<const V *, const V *> &l, const pair<const V *, const V *> &r) {
        try {
            set_intersection(
                    l.first, l.second, r.first, r.second, ThrowingIterator()
            );
    ...
    static pair<long double, set<V> >
    growThisEdge(Grouping &ging, const V edgeNumber, const long double &boost, bool randomized_p_in) {
        ...
        int sharedCommunities = ging.comm_count_per_edge(r, &(ging._g.neighbours(
                0).first[edgeNumber])); // a little time in here

        pair<long double, set<V> > grownSeed = growingSeed(ging, atoi(getenv("Lookahead") ?: "2"), seed,
                                                           make_pair(-10000000.0L,
                                                                     seed) // This score is too low, but then we don't expect a singleton community to come out best anyway! Only positive scores are used.
                , log2l(1.0L - (1.0L - ging._p_out) * powl(1.0L - ging._p_in, 1 + sharedCommunities))
                  - log2l(1.0L - (1.0L - ging._p_out) * powl(1.0L - ging._p_in, sharedCommunities)), 2, frontier, 1,
                                                           boost, randomized_p_in
        );
        return grownSeed;
    }
        ...
        for (V v = 0; v < g.vcount(); v++) {
            ...
            for (int addedBack = 0; _seedDeltas.size() > 0; addedBack++) {
                ...
                if (bestGroup.first > 0.0L) {
                    ...
                    if (num_positive ==
                        1) { // if just one was positive, then there's no point continuing, as the rest will only lose more score.
                        break;
                    }
