...
namespace graph_loading {
    ...
    template<class Name>
    static void loadBloomGraphMMAP(bloomGraph<Name> &bg, const char *fileName);
    ...
    static void populateNameToIDHash(
            SimpleIntGraph &bg); // A hash used temporarily during graph loading to test if a given edge has new nodes in it.
    static void populateNameToIDHash(
            SimpleStringGraph &bg); // A hash used temporarily during graph loading to test if a given edge has new nodes in it.
    static void
    addNeighbour(VertexIDType edge_source, VertexIDType edge_target, VertexIDType _defaultVertexID, V &_edges_added,
                 SimpleIntGraph &_bg);
    ...
    template<class N>
    void DegreeIncrementer(pair<N, V> &p) { ++p.second; }

    template<class N>
    void DegreeDecrementer(pair<N, V> &p) { --p.second; }

    enum state_flag {
        LOADING_NONE, LOADING_NODES, LOADING_DEGREEMAP, LOADING_EDGES, LOADING_COMPLETE
    };
    struct lsdkfj {
    ...
    void sigUSR1_handler(int) {
        Pn("SIGUSR1: %20s edge_counter %Ld", sigUSR1_state.state == LOADING_NONE ? "LOADING_NONE"
        ...
        }
        {
            ...
            Foreach(edge, roe2) {
                        ...
                        if (edge.first != edge.second) {
                            ...
                            bool was_inserted2 = edges.insert(
                                    edge).second; // we have to add it both ways here. Let's not worry about the efficiency of this yet!
                            assert(was_inserted == was_inserted2);

                            if (was_inserted) {
                                degree.get<NameTag>().modify(
                                        degree.get<NameTag>().find(edge.first), DegreeIncrementer<Name>
                                );
                                degree.get<NameTag>().modify(
                                        degree.get<NameTag>().find(edge.second), DegreeIncrementer<Name>
                                );
                            }
                        }
                    }
        }
        {
            ...
            if (option_maxDegree != 0) { // 0 will mean do nothing, the default, don't delete any edges.
                ...
                if (option_maxDegree == -1) {
                    ...
                    Pn("Deleting edges such that max degree (%d) equals TWICE average (uncorrected) degree (%g)",
                       max_degree, double(edges.size()) / names->size());
                } else {
                ...
                while (degree.get<DegreeTag>().begin()->second > max_degree) {
                    ...
                    while (i != edges.end() && i->first == n) {
                        ...
                        if ((/*deg>7?criteria*criteria:*/criteria) >= to_survive) {
                            ...
                            degree.get<NameTag>().modify(degree.get<NameTag>().find(delete_this),
                                                         DegreeDecrementer<Name>);
...
#define BLOOM_SIZE 800000000 // 100MB should be useful, and not a waste of memory
         ...
         */
        struct nested {
            static void addNewOrIncreaseDegree(vector<SimpleIntGraph::Name> &_vertices, const V _numSorted,
                ...
                decltype(_vertices.end()) lb = lower_bound(_vertices.begin(), splitHere, addThis);
