...
def write_edge2cid(e2c, filename, delimiter="\t"):
    ...
    with open(directory + os.sep + filename + ".edge2comm.txt", 'w') as f:
    ...
    with open(directory + os.sep + filename + ".comm2edges.txt", 'w') as f:
        with open(directory + os.sep + filename + ".comm2nodes.txt", 'w') as g:
...
def write_dendro(filename, orig_cid2edge, linkage):
    ...
    if not os.path.exists(directory):
        os.makedirs(directory)
    with open(directory + os.sep + filename + '.cid2edge.txt', 'w') as fout:
        for cid, e in orig_cid2edge.iteritems():
            fout.write("%d\t%s,%s\n" % (cid, str(e[0]), str(e[1])))

    with open(directory + os.sep + filename + '.linkage.txt', 'w') as fout:
        for x in linkage:
            fout.write('%s\n' % '\t'.join(map(str, x)))
