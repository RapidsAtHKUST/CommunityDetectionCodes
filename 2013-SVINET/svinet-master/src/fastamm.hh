...
//#define PRECISION_SAMPLE 1

class FPhiComp {
public:
  FPhiComp(const Env &env, gsl_rng **r, 
	   const uint32_t &iter,
...
inline void
FPhiComp::reset(uint32_t p, uint32_t q, yval_t y, bool phifix)
{
  _p = p;
  _q = q;
  _y = y;
...
inline void
FPhiComp::update_phis(bool is_phi1)
{
...
inline void
FPhiComp::compute_Elogf(uint32_t p, uint32_t q, yval_t y,
		       uint32_t K, uint32_t T,
		       const Matrix &Elogbeta, Array &Elogf)
{
  Elogf.zero();
...
inline void
FPhiComp::update_phis_until_conv()
{
  double u = 1./_k;
...
class FastAMM {
public:
  FastAMM(Env &env, Network &network);
  ~FastAMM();
...
inline void
FastAMM::set_dir_exp(const Matrix &u, Matrix &exp)
{
...
inline void
FastAMM::set_dir_exp(uint32_t a, const Matrix &u, Matrix &exp)
{
  const double ** const d = u.data();
  double **e = exp.data();
...
inline void
FastAMM::estimate_pi(uint32_t p, Array &pi_p) const
{
...
inline void
FastAMM::estimate_pi(const vector<uint32_t> &nodes)
{
  const double ** const gd = _gamma.const_data();
  double **epid = _Epi.data();
...
inline void
FastAMM::get_Epi(uint32_t n, Array &Epi)
{
  const double ** const gd = _gamma.const_data();
...
inline double
FastAMM::estimate_bernoulli_rate(uint32_t k) const
{
...
inline void
FastAMM::estimate_beta(Array &beta) const
{
...
inline double
FastAMM::edge_likelihood(uint32_t p, uint32_t q, yval_t y) const
{
  Array pi_p(_k);
  Array pi_q(_k);
  estimate_pi(p, pi_p);
...
inline bool
FastAMM::edge_ok(const Edge &e) const
{
  if (e.first == e.second)
    return false;
...
inline void
FastAMM::get_random_edge(bool link, Edge &e) const
{
  if (!link) {
    e.first = 0;
    e.second = 0;
    ...
    // do {
    //   int v  = gsl_rng_uniform_int(_r, _n);
    //   const vector<uint32_t> *zeros = _network.sparse_zeros(v);
    //   if (zeros->size()) {
    // 	int u  = gsl_rng_uniform_int(_r, zeros->size());
    // 	uint32_t z = (*zeros)[u];
    // 	e.first = v;
    // 	e.second = z;
...
inline void
FastAMM::set_iter(uint32_t a, uint32_t iter)
{
  _iter_map[a] = iter;
}

inline uint32_t
FastAMM::get_iter(uint32_t a)
{
...
inline void
FastAMM::add_to_community_map(uint32_t k, uint32_t p)
{
...
inline void
FastAMM::get_similar_nodes2(uint32_t n, 
			    vector<uint32_t> &neighbors)
{
  Array a(_k);
  _Epi.slice(0, n, a);
...
inline void
FastAMM::get_similar_nodes(uint32_t a, 
			   vector<uint32_t> &neighbors)
{
...
inline void
FastAMM::get_top_p_communities(uint32_t a, uint32_t p,
				 vector<uint32_t> &u)
{
  Array g(_k);
  _gamma.slice(0, a, g);
...
inline void
FastAMM::mark_seen(uint32_t a, uint32_t b)
{
...
inline yval_t
FastAMM::get_y(uint32_t p, uint32_t q)
{
...
#ifdef PRECISION_SAMPLE
inline uint32_t
FastAMM::precision_ones() const
{
  return 1000; // (uint64_t)(_network.ones() * _env.precision_ratio);
}

inline uint32_t
FastAMM::precision_zeros() const
{
