...
void Weighted_kernel_k_means(CtrlType *ctrl, GraphType *graph, int nparts, idxtype *w, float *tpwgts, float ubfactor){
  ...
  // added by Joyce
  IFSET(ctrl->dbglvl, DBG_COARSEN, printf("sigma value (wkkm.c) : %f\n",sigma));
  ...
  do{
	...
	min_ind = me = where[i];
	min_dist = squared_sum[me]*squared_inv_sum[me] -2*inv_wi*linearTerm[me]*inv_sum[me] + sigma/w[i] - sigma/sum[me];
	
	for (k=0; k<me; k++){
	  dist = squared_sum[k]*squared_inv_sum[k] -2*inv_wi*linearTerm[k]*inv_sum[k] + sigma/w[i] + sigma/sum[k]; 
