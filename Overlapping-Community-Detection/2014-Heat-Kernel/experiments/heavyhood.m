function [setsizes vols cuts conds times indices] = heavyhood(A,numtrials)
% [setsizes vols cuts conds times indices] = heavyhood(A,numtrials)
...
for trial_num=1:numtrials
    tic; [dummy,conds(trial_num),cut(trial_num),vols(trial_num)] = hkgrow(A,indices(trial_num),'neighborhood',true,'debug',false);
    times(trial_num) = toc;
